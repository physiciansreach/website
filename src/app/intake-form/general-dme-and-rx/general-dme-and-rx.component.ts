...
export class GeneralDmeAndRxComponent implements OnInit {
  ...
  initQuestions() {
    ...
    this.questions.push(this.initQuestion('11', 'Do you have chronic heartburn or acid reflux?'));
    this.questions.push(this.initQuestion('12', 'Do you experience dry mouth?'));
    this.questions.push(this.initQuestion('13', 'Are you diabetic?'));
    this.questions.push(this.initQuestion('14', 'Do you take oral or insulin to treat diabetes?'));
    this.questions.push(this.initQuestion('15', 'Have you seen a doctor in the last 12 months?'));
    this.questions.push(this.initQuestion('16', 'Have you had a liver test or liver function test?'));
    this.questions.push(this.initQuestion('17', 'Do you have a history of coronary heart disease?'));
    this.questions.push(this.initQuestion('18', 'Do you suffer from any of the following fungal infections of the skin?'));
  }
  ...
  buildIntake(): IntakeForm {
    ...
    this.addAnswer(this.questions[17], this.form.controls['q18'].value);

    const intake = new IntakeForm();
    intake.intakeFormType = IntakeFormType.GeneralDmeAndRx;
